...
- **Recompute castaway ADP:** `yarn recompute-castaway-adp 51` (dry run; `--fixture <file>` reads local JSON; `--write` publishes `castaway_adp/{season_id}`). Reads every competition with the Admin SDK and counts only drafts promoted before the premiere; nothing runs it on a schedule yet.
