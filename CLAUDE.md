...
- **Remap castaway ids:** `yarn remap-castaway-ids 51` (dry run: verifies the committed mapping in `scripts/castaway-id-remaps/` against its pinned survivoR commit, reads production, writes a plan and field-level backup under `data/migration-output/`; `--write --plan <file> --project <id>` applies it, marking each document applied in the same transaction; `--rollback` undoes it; `--rewrite-season-file` moves the bundled season file to survivoR's ids). Writes production only with `--write`/`--rollback`, which need explicit authorization. See `docs/castaway-id-mapping.md` for the id model, controls and Season 51 runbook.
