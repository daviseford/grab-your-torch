...
- **Remap castaway ids:** `yarn remap-castaway-ids 51` (dry run: maps committed provisional ids to survivoR's at a pinned commit, reads production, writes a plan and field-level backup under `data/migration-output/`; `--write --plan <file> --project <id>` applies it once per document via a ledger, `--rollback` undoes it). See `docs/castaway-id-mapping.md` for the id model and Season 51 runbook.
